...
All of this lives in one place, `src/lib/components/charts/LineChartPanel.svelte`: the `lineStyle`/`casingStyle` functions (both take a `deemphasized` flag — true for series colored `colors.regionGray`, e.g. figure 2's eight region lines) and the `marks` snippet that renders the two strokes per series. Tuning knobs: line width is 2.5 for a highlighted line (2 on mobile), 1 — a hairline — for a de-emphasized one; casing width comes from `casingWidth(deemphasized, innerWidth)`, stated as the SHOULDER of white showing on each side: 1.25 highlighted, 0.375 de-emphasized, which would wash out in white if it carried a full-width opaque halo across that many overlapping lines — those also get `opacity: 0.7` on the casing.

Order matters as much as width: a casing paints over whatever was drawn before it, so the `marks` snippet renders every de-emphasized series first and the emphasized ones last (`emphasizedLast`), independent of the figure's own series order. Without it, figure 2's region lines nick the world line at each crossing. Removing the `curve` property brings back straight segments while keeping the casing.
