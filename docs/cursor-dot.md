...
**Contrast.** The accent rust `#c24c2c` is the one saturated hue on this cover
— the photo is a blue-grey sky and fjord — and it sits mid-dark, which is the
luminance that survives a photograph rather than a flat ground: it reads
against the bright sky at the top (4.8:1) and against the `black/65` scrim at
the bottom (4.4:1). Over a chapter's ground or a figure's white surface the dot
would need a dark hairline to survive, and that hairline is what makes it read
as a bordered bead rather than a mark.

Worth an eye-check on the photo's **mid-tones**, which is where a mid-dark dot
has the least room. On `main` the equivalent decision was easy because that
cover is uniformly dark under a `black/50–65` scrim; this one has a wide
luminance range. The portable rule under both is *the dot is whatever reads on
that branch's cover, and the halo is always the accent at 35%*.
...
no reading — a title, the author line, one link and a scroll arrow.
...
things that *point*, and on this cover the cursor is the only place it is
spent at all.
