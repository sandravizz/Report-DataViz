...
`CursorDot.svelte` replaces the system pointer with a solid white dot inside a
...
**Contrast**, and this is where the branch differs from `main`. There the dot
is the accent itself, because that accent is a bright green over a uniformly
dark cover. IW's blue is *dark* — 5.6:1 on white but only ~3.7:1 on black — and
this cover is a photo under a `black/55–70` scrim plus a 0.58 radial through
the middle: exactly the ground the blue does worst on. It is the same fact that
made the cover's type white and pushed the accent underlines over the photo to
3px with a deepened scrim beneath them.

So the dot takes the cover's own type colour and the halo keeps the accent.
The portable rule under both branches:

> The dot is whatever reads on that branch's cover; the halo is always the
> accent at 35%.

Over a chapter's ground or a figure's white surface a dot would need a dark
hairline to survive, and that hairline is what makes it read as a bordered bead
rather than a mark — one more reason the zone stops at the cover.
...
no reading — a title, a standfirst, a credit line and a scroll arrow.
...
things that *point*, and the credit underlines the cursor moves between are the
...
Interpretation button in `ChartDisplay.svelte`.

**Eye-check this one.** A dark blue at 35% over a dark photo is subtle by
construction. If the halo reads as nothing at all, raise the 35% in
`.cursor-halo` — and raise it *there only*: the two buttons share the number by
coincidence of taste, not by requirement, and they sit on white.

There is no ink hairline on the halo. The dot inside it already carries the
edge, and a stroke around a 28px translucent ring reads as a bordered bead.
