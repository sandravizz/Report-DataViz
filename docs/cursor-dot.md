...
**Contrast.** The cover is the one flat, known ground in the report: a cream
page with blue type and a single orange chevron. The accent is that orange —
the only warm thing on the screen — so a 9px disc of it is unmistakable with
nothing propping it up. Over a chapter's ground or a figure's white surface the
dot would need a dark hairline to survive, and that hairline is what makes it
read as a bordered bead rather than a mark.

On `main` the same component works for the opposite reason: that cover is a
photograph under a `black/50–65` scrim and the accent is a bright green. The
portable rule underneath both is *the dot is whatever reads on that branch's
cover, and the halo is always the accent at 35%*.
...
no reading — a headline, a masthead block, the institute's cover artwork and a
scroll chevron.
...
things that *point*, and the chevron the cursor shares this screen with is the
other place it is spent here.
...
There is no ink hairline on the halo. The ground under this cursor is always
the flat cream cover, so there is nothing for one to rescue and it would only
muddy the orange.
