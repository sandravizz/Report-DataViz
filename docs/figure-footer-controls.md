...
**This branch's accent is IW's blue `#0069b4`** — dark, which is the case
`main`'s version calls out: the hover text flips to `accent-content` (white
clears 5.7:1 on the blue; the branch's own ink reaches only 1.9:1), and the
glyph has to follow it with `group-hover:text-accent-content` or it stays dark
under the fill.
...
construction, and no tint of a colour can contrast with that colour. On `main` that idea is geometrically impossible: its accent is so light that an
accent glyph on its own tint measures 1.24:1. This branch's blue is dark enough
that an accent glyph on the tint would in fact be legible — so here the ink
glyph is a *choice*, not a constraint, and the reason to keep it is the one
above: at 11px a pill with everything inside it in one hue reads as a
highlighted word rather than as a button.

The other half of that section still holds and is the more useful half here: a
dark accent is a good *fill under light text*, which is why the hover flips to
`accent-content` rather than keeping the ink. If a branch genuinely wants a bright accent glyph, the ground under it
...
offset, cut for 20px body copy — the same geometry `decoration-2
underline-offset-4` sets wherever a utility does this job on a light ground.
(The rules over the cover photo are the exception at 3px; the blue is 5.6:1 on
white but only ~3.7:1 on black.) Both numbers are absolute rather than em-based
...
use the single `MUTED` constant (`rgba(27, 65, 96, 0.5)` = `base-content/50`),
...
rather than guessing from the hue. A very light accent (`main`'s `#0FFF95`, at
1.3:1 on white) makes a good fill with dark ink on top. This branch's blue
inverts that — hence `hover:text-accent-content!` here, plus
`group-hover:text-accent-content` on the glyph. `kiel-institute`'s orange looks
like the same case and is not: white reaches only 2.9:1 on it while the ink
reaches 6.1:1, so it keeps `text-base-content`. Measure, do not infer.
