...
| `sandraviz.com` | wordmark, same file — **not present on this branch** |
...
construction, and no tint of a colour can contrast with that colour. On `main` that idea is geometrically impossible — its accent is so light that
an accent glyph on its own tint measures 1.24:1. **On this branch it is not:**
`#c24c2c` on its own resting tint over white (`#f0d2ca`) measures **3.4:1**,
which clears the 3:1 an icon needs. So the ink glyph here is a *choice*, not a
constraint, and the reason to keep it is the one above: at 11px a pill with
everything inside it in one hue reads as a highlighted word rather than as a
button, and the black glyph at 14.8:1 is what makes it a control. If a branch genuinely wants a bright accent glyph, the ground under it
...
offset, cut for the 18px chapter copy. Both numbers are absolute rather than em-based
...
use the single `MUTED` constant (`rgba(0, 0, 0, 0.5)` = `base-content/50`),
...
1.3:1 on white) makes a good fill with dark ink on top. This branch's rust is
mid-dark and inverts that — hence `hover:text-accent-content!` here, plus
`group-hover:text-accent-content` on the glyph so it does not stay black under
the fill. `kiel-institute`'s orange looks like the same case and is not: white
reaches only 2.9:1 on it while the ink reaches 6.1:1, so it keeps
`text-base-content`. Measure, do not infer.
