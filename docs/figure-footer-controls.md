...
**This branch's accent is the institute's orange `#ff6a00`**, and it is worth
saying what was measured before copying the classes: a saturated accent
usually has to flip the hover text to `accent-content`, but not this one. White
on `#ff6a00` is only **2.9:1**, while the near-black ink is **6.1:1**. So the
hover keeps `hover:text-base-content`, and the theme's `accent-content: #fff`
is left for places where the orange carries larger type.
...
`#ff6a00` on its own resting tint over white (`#ffdabf`) is **2.2:1** — under
the 3:1 an icon needs, and it gets worse as the tint deepens, because the
ground climbs toward the glyph. Every combination fails.

The accent here is 2.9:1 on white, which is enough for a *fill* under dark ink
and not enough for a small *foreground* mark on anything pale. If a branch genuinely wants a bright accent glyph, the ground under it
...
class="… text-base-content/50
       ...
       hover:bg-base-200 hover:shadow-lg"
```
...
use the single `MUTED` constant (`rgba(29, 24, 21, 0.5)` = `base-content/50`),
...
rather than guessing from the hue. This branch's orange looks like a case for
`accent-content`, and is not: the ink clears 6:1 on it and white does not. A
genuinely dark accent — a deep blue, a navy — does invert it, and there
`hover:text-base-content` fails and the hover text has to become
`hover:text-accent-content!`, with the glyph moved to
`group-hover:text-accent-content` so it does not stay dark under the fill.
