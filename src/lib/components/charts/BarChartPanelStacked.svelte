<script>
  ...
  let maxYTick = 0;
  const yTicks = (scale) => {
    // Scale's own 5-step candidates by default; a figure can pass its own array
    // (e.g. to drop a tick that always lands inside a bar, with no gridline).
  ...
  // Direct labels instead of a legend, on every viewport (house rule: they
  // never collapse into a bottom legend on mobile) — each series named beside
  // its segment on the last bar, in its own color.
    ...
    return pair.series.map((s) => {
      ...
      return {
        ...
        props: {
          ...
          // hue (the region ramp) pass `directLabelFill` to ink them uniformly.
          label: {
  ...
  // Stack total above every bar. Percent charts skip it (always a flat 100%).
  // Only the last bar spells out "million"; earlier ones are bare numbers
  // rounded to 1dp, to keep floating-point summing noise off screen. Sub-
  // million bars are the exception — rounding 0.95 to "1" would collide with
  // the "1" gridline — so those use formatMillions() too ("950 thousand").
  ...
  const growthArrow = $derived.by(() => {
    ...
    return {
      ...
      targetY: lastTotal,
      // Same rounding as the bars' own totals, so the printed gain always
      // matches the difference between the two printed numbers.
  ...
  // bandPadding is passed only so BarChart sees it non-null (its flag to
  // auto-baseline the value axis at 0); the real layout comes from xScale.
...
<!-- pair.percent switches to a 100% stacked layout: bars normalized per band
     (d3's stackOffsetExpand), y axis 0–100%, tooltip keeps the raw values. -->
<BarChart
  data={pair.data}
  x={pair.xKey}
  series={pair.series}
...
>
  {#snippet belowMarks()}
    <!-- Hatched projection band (a scenario year like "2035 STEPS"), full
         plot height over that column, behind the bars. -->
  ...
  {#snippet aboveMarks({ context })}
    ...
    {/each}
    <!-- Half the band width plus a gap starts the text just clear of the last
         bar's right edge; pair.endLabelGap widens it. -->
    ...
    {/each}
    {#if growthArrow}
      <!-- Bar-edge to bar-edge (not centre to centre), lifted clear of the
           totals. Raw Link/Text, not AnnotationPoint, to place the label. -->
      {@const lift = -10}
