<script>
  ...
  //
  //   1. Contrast. This cover is the one flat, known ground in the report: a
  //      cream page with blue type and a single orange chevron. The accent is
  //      that orange, and it is the only warm thing on the screen, so a 9px
  //      disc of it is unmistakable without a hairline propping it up. Over a
  //      chapter's ground or a figure's white surface — both of which carry
  //      charts and running text — it would need one, and a hairline is what
  //      makes a dot read as a bordered bead rather than a mark.
  ...
  //      neither. It has a headline, a masthead block, the institute's cover
  //      artwork and a scroll chevron.
  //
  ...
  // chevron it shares this screen with is the other place it is spent here.
  //
...
<style>
  ...
  .cursor-halo {
    ...
    /* accent/35 — see the note in the script block before changing it. No ink
       hairline here: the ground under this cursor is always the flat cream
       cover, so there is nothing for one to rescue and it would only muddy
       the orange. */
