<script>
  ...
  //
  //   1. The accent is 1.3:1 on white, so anywhere over the chapter ground or
  //      a figure surface it needs a dark hairline to survive — which is what
  //      made it read as a bordered bead. The cover is a black-and-white photo
  //      under a black/50-65 scrim, uniformly dark, so the accent reads at
  //      full strength with no help at all.
  ...
  //      neither. It has a title, two credit links and a scroll arrow.
  //
  ...
  // not bent: `--color-accent` is reserved for things that POINT, and the two
  // credit links it sits between on this cover are the other place it is spent.
  //
...
<style>
  ...
  .cursor-halo {
    ...
    /* accent/35 — see the note in the script block before changing it. No ink
       hairline here: the ground under this cursor is always the dark cover, so
       there is nothing for one to rescue and it would only muddy the green. */
