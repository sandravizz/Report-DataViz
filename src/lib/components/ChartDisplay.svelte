<script>
  ...
  // Mobile tab labels: strip the prefix all pairs share (e.g. "Abbildung 2")
  // so the strip reads short labels instead of repeating the full number on
  // every tab. A one-pair group has nothing to strip, so it falls back to
  // the full number.
