...
>
  ...
  <!-- The one filled control in the report: a faint accent wash at rest that
       ...
       The hover keeps `text-base-content` even though the accent is a
       saturated orange, which normally calls for `accent-content`: measured,
       white on #ff6a00 is only 2.9:1 while the near-black ink is 6.1:1. The
       glyph stays dark for the same reason.
