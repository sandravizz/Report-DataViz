...
// like the on-screen figure, not a generic system-font screenshot. Graphik
// is the IEA's single corporate face (see src/styles/fonts.css); ink is the
// site's --color-base-content (plain black, tailwind.css).
...
export async function downloadFigureImage({ figureEl, title, subtitle, source, filename }) {
  ...
  await Promise.all([
    document.fonts.load(`500 16px "Graphik"`),
    document.fonts.load(`400 16px "Graphik"`),
  ]);
